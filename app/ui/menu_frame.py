...
class MenuFrame(ctk.CTkFrame):
    ...
    def __init__(self, master, on_gestionar_preguntas, on_iniciar_juego, on_cerrar_sesion):
    ...
    def _construir_ui(self):
        ...
        ctk.CTkLabel(self, text="Menu Principal", font=FUENTE_TITULO, text_color=COLOR_TEXTO).grid(
        ...
        ctk.CTkButton(
            ...
            fg_color=COLOR_PRIMARIO, command=self.on_gestionar_preguntas
        ).grid(row=2, column=0, pady=10)

        ctk.CTkButton(
            self, text="Iniciar Juego", width=280, font=FUENTE_NORMAL,
            fg_color=COLOR_SECUNDARIO, command=self.on_iniciar_juego
        ).grid(row=3, column=0, pady=10)

        ctk.CTkButton(
            self, text="Cerrar Sesión", width=280, font=FUENTE_NORMAL,
            fg_color="transparent", border_width=2, text_color=COLOR_TEXTO,
